...
/** 判断 SMTP 错误是否属于临时/网络抖动，可重试 */
function isRetryableError(err) {
  if (!err) return false;
  const code = err.code || '';
  const responseCode = err.responseCode || 0;
  const message = String(err.message || '').toLowerCase();
  const retryableCodes = ['ECONNRESET', 'ETIMEDOUT', 'ECONNREFUSED', 'ENOTFOUND', 'EPIPE', 'ECONNABORTED'];
  if (retryableCodes.includes(code)) return true;
  if (responseCode >= 400 && responseCode < 500 && responseCode !== 421) {
    // 4xx 临时错误一般可重试，但 421 单独处理
    return true;
  }
  if (responseCode === 421) return true;
  if (/timeout|reset|refused|unavailable|busy|try again|temporary|later/i.test(message)) return true;
  return false;
}

/** 发送单封邮件，失败时自动重试 */
async function sendOneWithRetry(transporter, mail, maxRetries = 2) {
  let lastErr;
...
async function sendItems(items, opts = {}) {
  const { smtp, interval = 3000, onEach, shouldStop } = opts;
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (shouldStop && shouldStop()) {
      // 已请求取消：剩余条目不再尝试，标记 canceled 返回（不计入 onEach，不落发送日志）
      for (let j = i; j < items.length; j++) {
        results.push({ ok: false, canceled: true, index: j, subject: items[j].subject, to: items[j].to });
      }
      break;
    }
    const mail = {
    ...
    try {
      const info = await sendOneWithRetry(transporter, mail);
      const rec = { ok: true, index: i, to: mail.to, messageId: info.messageId };
      results.push(rec);
      if (onEach) onEach({ index: i, ok: true, item: it, info });
    } catch (err) {
      const rec = { ok: false, index: i, to: mail.to, error: err.message };
      results.push(rec);
      if (onEach) onEach({ index: i, ok: false, item: it, error: err });
      // 遇到反垃圾/频率限制，暂停 30 秒后再继续下一封，避免连续触发。
      if (/554|DT:SPM|频率|too many|rate|limit/i.test(err.message)) {
        await sleep(30000);
      }
    }
